...
export function BlockDragOverlay({ containerRef }: Props) {
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      try {
        ...
        if (!disposable) {
          try {
            disposable = sheet.onScroll(refresh);
          } catch {
            /* not ready yet */
          }
        }
        ...
        /* Univer not fully initialized yet */
      }
    };
    ...
    const isCanvasReady = () => {
      ...
      return Array.from(containerRef.current.querySelectorAll("canvas")).some(
    ...
    const poll = setInterval(() => {
      refresh();
      if ((skeletonRef.current && isCanvasReady()) || ++attempts > 40)
        clearInterval(poll);
    }, 150);
  ...
  function handleDragStart(regionId: string, evt: React.MouseEvent) {
    ...
    const titleRect = getCellScreenRect(
      skeletonRef.current,
      scrollStateRef.current,
      geo.titleRow,
      geo.anchorCol,
    ...
    function onMove(e: MouseEvent) {
      ...
      const { row, col } = screenPosToCellIndex(
        skeletonRef.current,
        liveScroll,
        relX,
        relY,
      ...
      const valid = isDropValid(
        regionId,
        row,
        col,
        currentSpecRef.current?.regions ?? [],
      );
      setDrag((prev) =>
        prev ? { ...prev, ghostRow: row, ghostCol: col, isValid: valid } : null,
      );
    }
    ...
    function onUp() {
      ...
      setDrag((prev) => {
        if (
          prev?.isValid &&
          (prev.ghostRow !== geo?.titleRow || prev.ghostCol !== geo.anchorCol)
        ) {
  ...
  const blocks: Array<{
    region: (typeof currentSpec.regions)[number];
    geo: BlockGeometry;
  }> = [];
  ...
  return (
    ...
    >
      ...
      {blocks.map(({ region, geo }) => {
        ...
        const titleH = getCellScreenRect(
          sk,
          sc,
          geo.titleRow,
          geo.anchorCol,
        ...
        const badgeClass = isField
          ? "block-badge block-badge--field"
          : "block-badge";
        const overlayClass = isField
          ? "block-overlay block-overlay--field"
          : "block-overlay";
        const handleClass = isField
          ? "block-drag-handle block-drag-handle--field"
          : "block-drag-handle";
        const label = isField
          ? (region.label ?? region.binding ?? "")
          : (region.block_ref ?? "");
      ...
      {drag &&
        (() => {
          const region = currentSpec.regions.find(
            (r) => r.id === drag.regionId,
          );
          ...
          const ghostRect = getBlockScreenRect(
            ghostGeo,
            sk,
            sc,
            canvasOffsetRef.current,
